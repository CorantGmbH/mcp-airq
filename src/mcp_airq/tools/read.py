...
def _rows_to_series_set(
    ...
    """Convert raw device rows to a SeriesSet for chart rendering and export."""
    key = sensor.lower()
    zone, _ = _effective_timezone(timezone_name)
    points = []
    for row in data:
...
async def export_air_quality_history(
    ...
    OUTPUT FORMAT:
    - "csv" — UTF-8 CSV with timestamp, series label, metric, unit and value
    - "xlsx" — Excel workbook with the same tabular content
